...
class LanguagePairWithGraphDataset(FairseqDataset):
    """
    ...
    def __getitem__(self, index):
        ...
        enc_edge_links1 = torch.zeros(len(edge_item))
        enc_edge_links2 = torch.zeros(len(edge_item))
        enc_edge_ids = torch.zeros(len(edge_item))

        # initialize mask with one, one will be mask when graph attention
        graph_mask = torch.ones(len(src_item), len(edge_item))
        graph_mask_rev = torch.ones(len(src_item), len(edge_item))

        for i, (head, relation, tail) in enumerate(edge_item):
            # key is head node idx, relation is relation idx between head and tail, tail is tail node idx
            if int(head) == -1:
                continue
