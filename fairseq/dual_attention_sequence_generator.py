    ...
    def generate(
        ...
        **kwargs
        ...
        encoder_input1 = {
            ...
            if k == 'src_tokens' or k == 'src_lengths'
        }
        encoder_input2 = {
            k: v for k, v in sample['net_input'].items()
        ...
        src_tokens = encoder_input1['src_tokens']
        ...
        encoder_outs1, encoder_outs2 = model.forward_encoder(encoder_input1, encoder_input2)
        new_order = torch.arange(bsz).view(-1, 1).repeat(1, beam_size).view(-1)
        new_order = new_order.to(src_tokens.device).long()
        encoder_outs1, encoder_outs2 = model.reorder_encoder_out(encoder_outs1, encoder_outs2, new_order)
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                ...
                model.reorder_encoder_out(encoder_outs1, encoder_outs2, reorder_state)

            lprobs, avg_attn_scores = model.forward_decoder(
                tokens[:, :step + 1], encoder_outs1, encoder_outs2, temperature=self.temperature,
            )
...
class EnsembleModel(torch.nn.Module):
    ...
    def has_encoder(self):
        assert hasattr(self.models[0], 'encoder1') and hasattr(self.models[0], 'encoder2')
        return True
    ...
    @torch.no_grad()
    def forward_encoder(self, encoder_input1, encoder_input2):
        if not self.has_encoder():
            return None
        return [model.encoder1(**encoder_input1) for model in self.models], [model.encoder2(**encoder_input2) for model in self.models]

    @torch.no_grad()
    def forward_decoder(self, tokens, encoder_outs1, encoder_outs2, temperature=1.):
        if len(self.models) == 1:
            return self._decode_one(
                tokens,
                self.models[0],
                encoder_outs1[0] if self.has_encoder() else None,
                encoder_outs2[0] if self.has_encoder() else None,
        ...
        log_probs = []
        avg_attn = None
        for model, encoder_out1, encoder_out2 in zip(self.models, encoder_outs1, encoder_outs2):
    ...
    def _decode_one(
        self, tokens, model, encoder_out1, encoder_out2, incremental_states, log_probs,
        temperature=1.,
    ):
        if self.incremental_states is not None:
            decoder_out = list(model.decoder(tokens, encoder_out1, encoder_out2, incremental_state=self.incremental_states[model]))
        else:
            decoder_out = list(model.decoder(tokens, encoder_out1, encoder_out2))
    ...
    def reorder_encoder_out(self, encoder_outs1, encoder_outs2, new_order):
        if not self.has_encoder():
            return
        return [
            model.encoder1.reorder_encoder_out(encoder_out, new_order)
            for model, encoder_out in zip(self.models, encoder_outs1)
        ], [
            model.encoder2.reorder_encoder_out(encoder_out, new_order)
            for model, encoder_out in zip(self.models, encoder_outs2)
        ]
